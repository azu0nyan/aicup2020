...
object ActivateRepairLogic extends StrategyPart {
  ...
  override def getActions(implicit g: GameInfo): ActionMap = {
    g.needRepairBUildings.flatMap { b =>
      ...
      val workersClose = Pathfinding.findNClosestReachableToBuilding(b.position.x, b.position.y, b.entityType.size, maxWorkerPerHouse,
      ...
      workers.flatMap { w =>
        borderCells.filter(c => !g.reservedForMovementCells(c)).minByOption(c => strategy.distance(c , (w.position.toProd)))
          .flatMap { targetCell =>
            Pathfinding.shortestPath(w.position.toProd, targetCell).map {
              case Seq() => g.repair(w, b)
              case path =>
                g.paths += path //debug
                g.move(w, path.head)
            }
