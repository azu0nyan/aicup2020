...
package object strategy extends VecOps {
  ...
  def distanceFromSquare(bPos: (Int, Int), size: Int, to: (Int, Int)) = {
    val xDist = Math.max(Math.max(bPos.x  -1 - to.x, to.x - (bPos.x + size)), 0)
    val yDist = Math.max(Math.max(bPos.y - 1 - to.y, to.y - (bPos.y + size)), 0)
    if (bPos.x <= to.x && to.x < bPos.x + size) yDist
    else if (bPos.y <= to.y && to.y < bPos.y + size) xDist
    else xDist + yDist + 1
